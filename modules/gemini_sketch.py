    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if result_sketch is None:
                print(f"   [오류] 이미지가 생성되지 않았습니다. (소요 시간: {elapsed_time:.2f}초)")
                return None
            ...
            print(f"   >> 생성 성공! (소요 시간: {elapsed_time:.2f}초)")
            return final_sketch

        except Exception as e:
            elapsed_time = time.time() - start_time
            print(f"   [시도 {attempt}/{max_retries} 실패] 오류: {e} (소요 시간: {elapsed_time:.2f}초)")
            
            if attempt < max_retries:
                print(f"   {retry_delay}초 후 다시 시도합니다...")
                time.sleep(retry_delay)
            else:
                print("   [최종 실패] 모든 재시도 횟수를 초과했습니다.")
                return None
